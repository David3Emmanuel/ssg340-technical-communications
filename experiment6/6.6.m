...
baseLayer = [1 2; 3 4; 5 6];        % base 2D matrix
A = cat(3, baseLayer, baseLayer*2); % add a third dimension (2 layers)
...
fprintf('\n--- Task 2: Matrix Reshaping ---\n');
rowData = [1:6];
B = reshape(rowData, 2, 3);      % reshape into 2x3
disp('Reshaped matrix B:');
disp(B);

bPlusTen = B + 10;              % simple operation on reshaped matrix
disp('B + 10:');
disp(bPlusTen);
...
rowVec = [1 2 3];
tiledRows = repmat(rowVec, 4, 1);   % replicate row vector 4 times
disp('Tiled matrix (row vector repeated 4x):');
disp(tiledRows);

pascalTiledTwice = repmat(P, 1, 2); % tile Pascal matrix side by side
disp('Pascal matrix tiled twice horizontally:');
disp(pascalTiledTwice);
